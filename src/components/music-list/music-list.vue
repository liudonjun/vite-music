...
import Scroll from '../base/scroll copy/scroll.vue'
import SongList from '../base/song-list/song-list.vue'
import { storeToRefs } from 'pinia'
import { getLyric, getSongUrl } from '@/api/api.ts'
...
const emit = defineEmits(['onPullUp'])
...
const onPullUp = (bool: boolean) => {
  isPullUpLoad.value = bool
  emit('onPullUp', bool)
}

const selectItem = async (item: object, index: number) => {
...
const filterStyle = computed(() => {
  ...
  if (scrollY2 >= 0) {
    blur = Math.min(maxTranslateY.value / imageHeight2, scrollY2 / imageHeight2) * 10
  }
